 ...
 * NeoForge-style DeferredHolder bootstrap builds the object via a plain {@code Supplier} and only
 * registers it afterwards, so the Properties never learn their id. This context lets the resolver
 * publish the pending id for the current registration; constructor mixins on BlockBehaviour/Item
 * read it and call {@code Properties.setId(...)} before the engine reads the id.
 */
