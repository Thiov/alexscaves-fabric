...
public class HullbreakerEntity extends WaterAnimal implements IAnimatedEntity, KaijuMob, MultiPartEntity {
    ...
    public static final Predicate<LivingEntity> GLOWING_TARGET = (mob) -> {
        return mob.isInWater() && (mob.hasEffect(MobEffects.GLOWING) || mob.getType().builtInRegistryHolder().is(ACTagRegistry.GLOWING_ENTITIES) || mob.isPassenger() && mob.getVehicle() instanceof SubmarineEntity sub && sub.areLightsOn());
    };
    ...
    public static boolean checkHullbreakerSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnType, BlockPos pos, RandomSource randomSource) {
    ...
    private void populateDeathLootForHullbreaker(){
        ...
        if(damageSource != null){
            LootTable loottable = this.level().getServer().reloadableRegistries().getLootTable(this.getLootTable().orElseThrow());
            ...
            if (this.getLastHurtByPlayer() != null) {
                lootparams$builder = lootparams$builder.withParameter(LootContextParams.LAST_DAMAGE_PLAYER, this.getLastHurtByPlayer()).withLuck(this.getLastHurtByPlayer().getLuck());
            }
    ...
    public void tick() {
        ...
        boolean grounded = this.onGround() && !isInWater();
    ...
    protected SoundEvent getAmbientSound() {
        return isInWater() ? ACSoundRegistry.HULLBREAKER_IDLE.get() : ACSoundRegistry.HULLBREAKER_LAND_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return isInWater() ? ACSoundRegistry.HULLBREAKER_HURT.get() : ACSoundRegistry.HULLBREAKER_LAND_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return isInWater() ? ACSoundRegistry.HULLBREAKER_DEATH.get() : ACSoundRegistry.HULLBREAKER_LAND_DEATH.get();
    }
