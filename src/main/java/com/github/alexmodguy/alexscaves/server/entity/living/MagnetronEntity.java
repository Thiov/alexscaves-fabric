...
public class MagnetronEntity extends Monster implements MultiPartEntity {
    ...
    private static final EntityDataAccessor<CompoundTag> BLOCKSTATES = SynchedEntityData.defineId(MagnetronEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.COMPOUND_TAG.get());
    private static final EntityDataAccessor<CompoundTag> BLOCK_POSES = SynchedEntityData.defineId(MagnetronEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.COMPOUND_TAG.get());
    ...
    public static boolean checkMagneticCaveSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
        ...
        if (!this.isAlive() && shouldDropBlocks()) {
            if (this.isFormed()) {
                for (MagnetronPartEntity part : allParts) {
                    if (part.getBlockState() != null && com.github.alexmodguy.alexscaves.server.entity.util.LevelCompat.getGameRules(level()).get(net.minecraft.world.level.gamerules.GameRules.MOB_GRIEFING)) {
                        BlockPos placeAt = part.blockPosition();
                        while (!level().getBlockState(placeAt).isAir() && placeAt.getY() < level().getMaxY()) {
    ...
    private boolean shouldDropBlocks() {
        ...
        if (lastDamageSource != null) {
            return lastDamageSource.getEntity() != null || lastDamageSource.getDirectEntity() != null || this.getLastHurtByPlayer() != null;
        }
        return false;
    }
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        this.setFormed(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "Formed"));
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "Blockstates", 10)) {
            this.entityData.set(BLOCKSTATES, com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getCompound(compound, "Blockstates"));
        }
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "BlockPoses", 10)) {
            this.entityData.set(BLOCK_POSES, com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getCompound(compound, "BlockPoses"));
        }
    }
    ...
    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput compound) {
        ...
        com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putCompound(compound, "Blockstates", this.entityData.get(BLOCKSTATES));
        com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putCompound(compound, "BlockPoses", this.entityData.get(BLOCK_POSES));

    }

    public boolean hurtServer(net.minecraft.server.level.ServerLevel serverLevel, DamageSource damageSource, float f) {
        boolean prev = super.hurtServer(serverLevel, damageSource, f);
    ...
    private void tickMultipart() {
        ...
        if (this.isFunctionallyMultipart()) {
            ...
            if (this.isAlive() && bottom.distanceTo(ground) < 7 && ground.y > level().getMinY()) {
                if (diff.length() > 1) {
                    diff = diff.normalize();
                }
    ...
    private List<BlockState> getAllBlockStates() {
        ...
        if (data.contains("BlockData")) {
            ListTag listTag = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getList(data, "BlockData");
    ...
    private List<BlockPos> getAllBlockPos() {
        ...
        if (data.contains("BlockPos")) {
            ListTag listTag = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getList(data, "BlockPos");
            for (int i = 0; i < listTag.size(); ++i) {
                CompoundTag innerTag = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getCompound(listTag, i);
                com.github.alexmodguy.alexscaves.server.misc.NbtCompat.readBlockPos(innerTag, "pos").ifPresent(list::add);
            }
        }
        return list;
    }
    ...
    private void setAllBlockPos(List<BlockPos> list) {
        ...
        for (BlockPos pos : list) {
            ...
            posTag.put("pos", com.github.alexmodguy.alexscaves.server.misc.NbtCompat.writeBlockPos(pos));
            listTag.add(posTag);
        }
        ...
        if (!level().isClientSide()) {
            ...
            for (MagnetronPartEntity entity : this.allParts) {
                if (entity.getBlockState() == null && entity.getStartPosition() != null) {
                    ...
                    saved.add(state);
                    if (com.github.alexmodguy.alexscaves.server.entity.util.LevelCompat.getGameRules(level()).get(net.minecraft.world.level.gamerules.GameRules.MOB_GRIEFING) && !level().isClientSide()) {
    ...
    public void spawnGroundEffects() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int i1 = 0; i1 < 20 + random.nextInt(12); i1++) {
                ...
                if (state.isSolid()) {
                    if (level().isClientSide()) {
                        com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, state), true, center.x + extraX, ground.getY() + extraY, center.z + extraZ, motionX, motionY, motionZ);
                    }
                }
            }
        }
            ...
            if (attackPose == AttackPose.SLAM) {
                ...
                for (Entity entity : MagnetronEntity.this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
                    if (!isAlliedTo(entity) && !(entity instanceof MagnetronEntity)) {
                        entity.hurtOrSimulate(damageSources().mobAttack(MagnetronEntity.this), 2 + leftDmg + rightDmg);
                        launch(entity, true);
                    }
                }
                ...
                target.hurtOrSimulate(damageSources().mobAttack(MagnetronEntity.this), 2 + leftDmg);
                launch(target, magnet);
            } else if (attackPose == AttackPose.RIGHT_PUNCH) {
                BlockState state = getStateForHand(false);
                boolean magnet = state.is(ACBlockRegistry.AZURE_MAGNET.get()) || state.is(ACBlockRegistry.SCARLET_MAGNET.get());
                target.hurtOrSimulate(damageSources().mobAttack(MagnetronEntity.this), 2 + rightDmg);
                launch(target, magnet);
            }
        }
