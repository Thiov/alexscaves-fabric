...
public class ForsakenEntity extends Monster implements IAnimatedEntity, ShakesScreen {
    ...
    public static final net.minecraft.world.entity.ai.targeting.TargetingConditions.Selector TARGETING = (mob, serverLevel) -> {
                ...
                if (this.getAnimationTick() == 12) {
                    ...
                    for (int i1 = 0; i1 < particleCount; i1++) {
                        ...
                        if (groundState.isSolid()) {
                            if (level().isClientSide()) {
                                com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, groundState), true, smashPos.x + extraX, ground.getY() + extraY, smashPos.z + extraZ, motionX, motionY, motionZ);
                            }
                        }
                    }
                }
    ...
    public static boolean checkForsakenSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
