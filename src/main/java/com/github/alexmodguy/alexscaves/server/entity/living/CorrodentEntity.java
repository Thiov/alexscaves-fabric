...
public class CorrodentEntity extends Monster implements ICustomCollisions, IAnimatedEntity, MultiPartEntity {
        ...
        if (isDigging() && surfacePosition != null) {
            if (level().isClientSide() && isMoving()) {
                ...
                if (surfaceState.isSolid()) {
                    ...
                    com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, surfaceState), true, head.x, head.y, head.z, random.nextFloat() - 0.5F, random.nextFloat() + 0.5F, random.nextFloat() - 0.5F);
                    for (int i = 0; i < 4 + random.nextInt(4); i++) {
                        ...
                        com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, surfaceState), true, offset.x + head.x, offset.y + head.y, offset.z + head.z, (random.nextFloat() - 0.5F) * 0.2F + offset.x, (random.nextFloat() - 0.5F) * 0.2F + offset.y, (random.nextFloat() - 0.5F) * 0.2F + offset.z);
                    }
                }
                if (onState.isSolid()) {
                    for (int i = 0; i < 2 + random.nextInt(2); i++) {
                        com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, onState), true, this.getRandomX(0.8F), this.getRandomY(), this.getRandomZ(0.8F), (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F);
                    }
                }
            }
        }
        ...
        if (b == 77) {
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                if (groundState.isSolid()) {
                    if (level().isClientSide()) {
                        com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, groundState), true, this.getX() + extraX, ground.getY() + extraY, this.getZ() + extraZ, motionX, motionY, motionZ);
                    }
                }
            }
        } else {
    ...
    private Vec3 calculateLightAbovePosition() {
        ...
        while (mutableBlockPos.getY() < level().getMaxY() && level().getBlockState(mutableBlockPos).isSuffocating(level(), mutableBlockPos)) {
    ...
    public boolean isInvulnerableTo(DamageSource damageSource) {
        return super.isInvulnerableToBase(damageSource) || damageSource.is(DamageTypes.IN_WALL);
    }
    ...
    public static boolean checkCorrodentSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    static class DiggingNodeEvaluator extends FlyNodeEvaluator {
        ...
        public PathType getPathType(PathfindingContext context, int x, int y, int z) {
            ...
            if (def == PathType.LAVA || def == PathType.OPEN || def == PathType.WATER || def == PathType.WATER_BORDER || def == PathType.DAMAGING_IN_NEIGHBOR || def == PathType.FIRE || def == PathType.POWDER_SNOW) {
                return PathType.BLOCKED;
            }
            return isSafeDig(context.level(), pos) && pos.getY() > context.level().getMinY() ? PathType.WALKABLE : PathType.BLOCKED;
        }
    }
