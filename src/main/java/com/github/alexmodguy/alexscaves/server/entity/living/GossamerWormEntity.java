...
public class GossamerWormEntity extends WaterAnimal implements Bucketable, MultiPartEntity {
    ...
    public void tick() {
        ...
        this.tickMultipart();
        float targetPitch = isInWater() ? Mth.clamp((float) this.getDeltaMovement().y * 25, -1.4F, 1.4F) * -(float) (180F / (float) Math.PI) : 0;
    ...
    public boolean hurtServer(net.minecraft.server.level.ServerLevel serverLevel, DamageSource damageSource, float damageValue) {
        boolean sup = super.hurtServer(serverLevel, damageSource, damageValue);
        if (sup) {
    ...
    public static boolean checkGossamerWormSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnType, BlockPos pos, RandomSource randomSource) {
    ...
    private void doInitialPosing(LevelAccessor world) {
        ...
        while (!world.getFluidState(down).isEmpty() && down.getY() > world.getMinY()) {
            down = down.below();
        }
