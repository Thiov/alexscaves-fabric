...
public class LicowitchAttackGoal extends Goal {
    ...
    private void hexAttack(LivingEntity target) {
        ...
        if (summonSpace && licowitch.getRandom().nextBoolean()) {
            ...
            if (summonSpot != null) {
                ...
                if (licowitch.level() instanceof ServerLevel serverLevel) {
                    summoned.finalizeSpawn(serverLevel, ((net.minecraft.server.level.ServerLevel) licowitch.level()).getCurrentDifficultyAt(BlockPos.containing(summonSpot)), EntitySpawnReason.MOB_SUMMONED, null);
                }
        ...
        if (!flag) {
            ...
            SugarStaffHexEntity sugarStaffHexEntity = ACEntityRegistry.SUGAR_STAFF_HEX.get().create(licowitch.level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
    ...
    private void peppermintAttack(LivingEntity target) {
        ...
        for (int i = 0; i < spawnIn; i++) {
            SpinningPeppermintEntity spinningPeppermintEntity = ACEntityRegistry.SPINNING_PEPPERMINT.get().create(licowitch.level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
