...
public class TremorzillaEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, KaijuMob, ActivatesSirens, ITallWalker, MultiPartEntity, RendersOwnRider {
    ...
    public TremorzillaEntity(EntityType entityType, Level level) {
        ...
        dummyExplosion = com.github.alexmodguy.alexscaves.server.misc.ExplosionCompat.createDummy(level(), this);
    }
        ...
        if(!level().isClientSide() && this.getAnimation() == ANIMATION_CHEW && this.getAnimationTick() == 34 && lastFedPlayer != null){
            if(!this.isTame()){
                ...
                if (this.getTameAttempts() >= 4 && getRandom().nextInt(3) == 0) {
                    ...
                    com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.clearRestriction(this);
    ...
    public void calculateEntityAnimation(boolean flying) {
        ...
        com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.updateWalkAnimation(walkAnimation, f2, 0.4F);
    }
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.setCharge(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "Charge"));
        this.setSpikesDownAmount(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getFloat(compound, "SpikesDownAmount"));
        this.wantsToUseBeamFromServer = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "ServerBeamTrigger");
        this.setTameAttempts(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "TameAttempts"));
    }
    ...
    public boolean hurtEntitiesAround(Vec3 center, float radius, float damageAmount, float knockbackAmount, boolean radioactive, boolean hurtsOtherKaiju, boolean stretchY) {
        ...
        for (LivingEntity living : level().getEntitiesOfClass(LivingEntity.class, aabb, EntitySelector.NO_CREATIVE_OR_SPECTATOR)) {
            if (!living.is(this) && !this.isAlliedTo(living) && living.getType() != this.getType() && living.distanceToSqr(center.x, stretchY ? living.getY() : center.y, center.z) <= radius * radius && (!radioactive || canEntityBeHurtByBeam(living, center)) && (hurtsOtherKaiju || !(living instanceof KaijuMob))) {
                if (living.hurtOrSimulate(damageSource, damageAmount)) {
                    flag = true;
