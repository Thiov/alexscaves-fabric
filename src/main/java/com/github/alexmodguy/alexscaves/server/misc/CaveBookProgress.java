...
public class CaveBookProgress {
    ...
    private CaveBookProgress(CompoundTag tag) {
        tag.getList("Pages").ifPresent(listTag -> {
            for (int i = 0; i < listTag.size(); ++i) {
                com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getCompoundOptional(listTag, i).ifPresent(compoundTag -> unlockedPages.put(
                    compoundTag.getStringOr("Category", ""),
                    Subcategory.getByOrdinal(compoundTag.getIntOr("SubCategory", 0))
                ));
            }
        });
    }
    ...
    public static CaveBookProgress getCaveBookProgress(Player player) {
        ...
        CompoundTag savedTag = tag.getCompound(PLAYER_CAVE_BOOK_PROGRESS_TAG).orElseGet(CompoundTag::new);
        return new CaveBookProgress(savedTag);
    }
    ...
    public static void saveCaveBookProgress(CaveBookProgress caveBookProgress, Player player) {
        ...
        if (!player.level().isClientSide()) {
            ...
            PacketDistributor.sendToServer(new UpdateCitadelTagMessage(player.getId(), tag));
        }
    }
    ...
    public boolean unlockNextFor(String biomeCategory) {
        ...
        if (Subcategory.canUnlockNext(biomeCategory, prev)) {
            Subcategory unlocked = AlexsCaves.COMMON_CONFIG.onlyOneResearchNeeded.get()
                ? Subcategory.getLastUnlockableFor(biomeCategory)
                : Subcategory.getByOrdinal(prev + 1);
    ...
    public enum Subcategory {
        ...
        public static boolean canUnlockNext(String category, int currentLevel) {
            ...
            return next.limitedTo.length == 0 || Arrays.stream(next.limitedTo)
                .anyMatch(biomeResourceKey -> biomeResourceKey.identifier().toString().equals(category));
        }

        public static Subcategory getLastUnlockableFor(String category) {
            Subcategory subcategory = SECRETS;
            if (subcategory.limitedTo.length > 0 && Arrays.stream(subcategory.limitedTo)
                .anyMatch(biomeResourceKey -> biomeResourceKey.identifier().toString().equals(category))) {
                return subcategory;
            }
