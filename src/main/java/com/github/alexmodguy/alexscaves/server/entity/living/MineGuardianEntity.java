...
public class MineGuardianEntity extends Monster {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ANCHOR_UUID = SynchedEntityData.defineId(MineGuardianEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    protected SoundEvent getAmbientSound() {
        return isInWater() ? ACSoundRegistry.MINE_GUARDIAN_IDLE.get() : ACSoundRegistry.MINE_GUARDIAN_LAND_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return isInWater() ? ACSoundRegistry.MINE_GUARDIAN_HURT.get() : ACSoundRegistry.MINE_GUARDIAN_LAND_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return isInWater() ? ACSoundRegistry.MINE_GUARDIAN_DEATH.get() : ACSoundRegistry.MINE_GUARDIAN_LAND_DEATH.get();
    }
    ...
    public void tick() {
        ...
        if (this.isExploding()) {
            if (explodeProgress >= 10.0F) {
                ...
                Explosion.BlockInteraction blockinteraction = net.neoforged.neoforge.event.EventHooks.canEntityGrief(level(), this) ? com.github.alexmodguy.alexscaves.server.entity.util.LevelCompat.getGameRules(level()).get(net.minecraft.world.level.gamerules.GameRules.MOB_EXPLOSION_DROP_DECAY) ? Explosion.BlockInteraction.DESTROY_WITH_DECAY : Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.KEEP;
                MineExplosion explosion = new MineExplosion(level(), this, this.getX(), this.getY(0.5), this.getZ(), 5.0F, this.isInWater(), blockinteraction);
                explosion.explode();
        ...
        if (!level().isClientSide()) {
            ...
            if (this.isScanning()) {
                ...
                if (timeSinceHadTarget == 0 || !this.isInWater()) {
                    this.setEyeClosed(false);
                } else if (this.isInWater() && (timeSinceHadTarget > maxSleepTime && j > 200 || this.hurtTime > 0)) {
                ...
                if (this.isInWater() && timeSinceHadTarget > 100) {
    ...
    public static boolean checkMineGuardianSpawnRules(EntityType entityType, ServerLevelAccessor level, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
        if (!level.getFluidState(blockPos).is(FluidTags.WATER)) {
            return false;
        } else {
            boolean flag = level.getDifficulty() != Difficulty.PEACEFUL && isDarkEnoughToSpawn(level, blockPos, randomSource) && (mobSpawnType == EntitySpawnReason.SPAWNER || level.getFluidState(blockPos).is(FluidTags.WATER));
            if (randomSource.nextInt(10) == 0 && blockPos.getY() < level.getSeaLevel() - 50 && flag) {
                ...
                pos.set(blockPos);
                while (!level.getFluidState(pos).isEmpty() && pos.getY() > level.getMinY()) {
                    pos.move(0, -1, 0);
                }
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.hasUUID(compound, "AnchorUUID")) {
            this.setAnchorUUID(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getUUID(compound, "AnchorUUID"));
        }
        this.setMaxChainLength(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "MaxChainLength"));
        this.scanTime = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "ScanTime");
        this.setEyeClosed(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "EyeClosed"));
    }

    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput compound) {
        super.addAdditionalSaveData(compound);
        if (this.getAnchorUUID() != null) {
            com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putUUID(compound, "AnchorUUID", this.getAnchorUUID());
        }
    ...
    private class MeleeGoal extends Goal {
        ...
        public void tick() {
            ...
            if (target != null) {
                ...
                if (dist > 2.0F) {
                    if (MineGuardianEntity.this.isInWater()) {
