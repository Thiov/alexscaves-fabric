...
public class DarknessArmorItem extends ArmorItem implements CustomArmorPostRender, KeybindUsingArmor, UpdatesStackTags {
    ...
    public static boolean canChargeUp(ItemStack itemStack) {
        ...
        return com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(tag, "CanCharge");
    }
    ...
    public static float getMeterProgress(ItemStack cloak) {
        ...
        CompoundTag tag = customData.copyTag();
        return com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "CloakCharge") / (float) AlexsCaves.COMMON_CONFIG.darknessCloakChargeTime.get();
    }
        ...
        if (stack.is(ACItemRegistry.CLOAK_OF_DARKNESS.get()) && entity instanceof LivingEntity living) {
            ...
            if (living.getItemBySlot(EquipmentSlot.CHEST) == stack && entity instanceof Player player) {
                if (!level.isClientSide()) {
                    ...
                    CompoundTag tag = customData.copyTag();
                    int charge = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "CloakCharge");
                    boolean flag = false;
            ...
            if (living.getItemBySlot(EquipmentSlot.CHEST) == stack) {
                ...
                CompoundTag tag = customData.copyTag();
                if (!level.isClientSide()) {
                    long lastLightTimestamp = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getLong(tag, "LastLightTimestamp");
                    long lastEquipMessageTime = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getLong(tag, "LastEquipMessageTime");
                    ...
                    if (lastEquipMessageTime <= 0 && entity instanceof Player player) {
                        ...
                        player.sendOverlayMessage(Component.translatable("item.alexscaves.cloak_of_darkness.equip"));
                        modified = true;
                    }
                    if (modified) {
                        stack.set(DataComponents.CUSTOM_DATA, CustomData.of(tag));
    ...
    public void onKeyPacket(Entity wearer, ItemStack itemStack, int key) {
        if (wearer instanceof LivingEntity living && canChargeUp(living, false)) {
            ...
            player.sendOverlayMessage(Component.translatable("item.alexscaves.cloak_of_darkness.requires_darkness"));
        }
    }
    ...
    public Identifier getArmorTexture(ItemStack stack, Entity entity, EquipmentSlot slot, Object layer, boolean innerModel) {
        return Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/armor/darkness_armor.png");
    }
}
