...
public class CaveMapRenderer {
    ...
    public static final Identifier MAP_BACKGROUND = Identifier.withDefaultNamespace("textures/map/map_background.png");
    public static final RenderType CAVE_MAP_PLAYER_TEXTURE = net.minecraft.client.renderer.rendertype.RenderTypes.text(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/misc/map/cave_map_player.png"));
    public static final RenderType CAVE_MAP_PLAYER_DIRECTION_TEXTURE = net.minecraft.client.renderer.rendertype.RenderTypes.text(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/misc/map/cave_map_player_direction.png"));
    ...
    public CaveMapRenderer(BlockPos target, int[] mapBiomes, long seed, boolean transparent, int index) {
        ...
        this.texture = new DynamicTexture(() -> "alexscaves:cave_map", 128, 128, true);
        // 26.1: TextureManager#register takes an Identifier (not a String) and returns void.
        Identifier resourcelocation = Identifier.withDefaultNamespace("cave_map/" + index + (transparent ? "" : "_frame"));
        Minecraft.getInstance().getTextureManager().register(resourcelocation, this.texture);
        this.renderType = net.minecraft.client.renderer.rendertype.RenderTypes.text(resourcelocation);
        updateTexture();
        updateLabels();
    }
    ...
    private void updateTexture() {
        net.minecraft.core.Registry<Biome> registry = (net.minecraft.core.Registry<Biome>) Minecraft.getInstance().level.registryAccess().lookupOrThrow(Registries.BIOME);
        if (mapBiomes.length >= 128 * 128) {
            for (int i = 0; i < 128; ++i) {
                for (int j = 0; j < 128; ++j) {
                    ...
                    com.github.alexmodguy.alexscaves.client.render.compat.NativeImageCompat.setPixelRGBA(this.texture.getPixels(), j, i, FastColor.ABGR32.color(alpha, b, g, r));
                }
            }
        }
        this.texture.upload();
    ...
    private int getBiomeColor(Holder<Biome> biome, int u, int v) {
        ...
        if (biome.is(BiomeTags.IS_OCEAN) && biome.value().coldEnoughToSnow(BlockPos.ZERO, 0)) {
        ...
        if (biome.is(BiomeTags.IS_MOUNTAIN) && biome.value().coldEnoughToSnow(BlockPos.ZERO, 0)) {
            return DefaultMapBackgrounds.SNOWY_MOUNTAIN.getMapColor(u, v);
        }
        if (biome.is(BiomeTags.IS_TAIGA) && biome.value().coldEnoughToSnow(BlockPos.ZERO, 0)) {
        ...
        if (biome.value().coldEnoughToSnow(BlockPos.ZERO, 0) && !biome.is(BiomeTags.IS_TAIGA)) {
    ...
    public void renderLabels(PoseStack poseStack, MultiBufferSource multiBufferSource, int light) {
        for (BiomeLabel label : labels) {
            ...
            Component component = Component.translatable(getBiomeString(label.biome.identifier().toString()));
