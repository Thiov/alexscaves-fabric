...
public class VoidBeingCloudParticle extends net.minecraft.client.particle.TextureSheetParticle implements RenderInWorldParticle {
    ...
    public VoidBeingCloudParticle(ClientLevel world, double x, double y, double z, int size, int target, int totalTendrils) {
        ...
        dynamicTexture = new DynamicTexture(() -> "alexscaves:void_being_cloud", textureSize, textureSize, true);
        id = currentlyUsedTextures;
        // 26.1: TextureManager#register takes an Identifier and returns void.
        Identifier resourcelocation = Identifier.fromNamespaceAndPath("alexscaves", "void_particle/void_cloud_" + id);
    ...
    private void updateTexture() {
        ...
        for (int i = 0; i < textureSize; ++i) {
            for (int j = 0; j < textureSize; ++j) {
                ...
                if (alpha < 0) {
                    com.github.alexmodguy.alexscaves.client.render.compat.NativeImageCompat.setPixelRGBA(this.dynamicTexture.getPixels(), j, i, 0);
                } else {
                    com.github.alexmodguy.alexscaves.client.render.compat.NativeImageCompat.setPixelRGBA(this.dynamicTexture.getPixels(), j, i, FastColor.ARGB32.color((int) Math.min(alpha * 255, 255), black, black, black));
                }
            }
        }
