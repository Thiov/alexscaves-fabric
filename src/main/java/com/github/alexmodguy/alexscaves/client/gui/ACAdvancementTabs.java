...
public class ACAdvancementTabs {
    ...
    public static boolean isAlexsCavesWidget(AdvancementHolder holder) {
        return holder != null && holder.id().getNamespace().equals(AlexsCaves.MODID);
    }

    public static void renderTabBackground(GuiGraphicsExtractor guiGraphics, int topX, int topY, DisplayInfo displayInfo,
    ...
    private static void renderTabBackgroundForType(GuiGraphicsExtractor guiGraphics, int topX, int topY, float partialTick,
            ...
            for (int parallaxX = -1; parallaxX <= (windowWidth + 128) / 128; parallaxX++) {
                for (int parallaxY = -1; parallaxY <= (windowWidth + 128) / 128; parallaxY++) {
                    ColorBlitHelper.blitWithColor(guiGraphics, type.background, parallaxX * 128 + i / 4,
                        parallaxY * 128 + j / 4, 128, 128, 1F, 1F, 1F, alpha);
                    ColorBlitHelper.blitWithColor(guiGraphics, type.midground, parallaxX * 128 + i / 2 - 1,
                        parallaxY * 128 + j / 2, 128, 128, 1F, 1F, 1F, alpha);
                }
            }
        }
        ...
        for (int relativeBlockX = -2; relativeBlockX <= screenWidthInBlocks; relativeBlockX++) {
            for (int relativeBlockY = -2; relativeBlockY <= screenHeightInBlocks; relativeBlockY++) {
                ...
                ColorBlitHelper.blitWithColor(guiGraphics, type.baseStone, 16 * relativeBlockX + scrollPixelOffsetX,
                    16 * relativeBlockY + scrollPixelOffsetY, 16, 16, 1F, 1F, 1F, alpha);
            }
        }
    }
    ...
    public enum Type {
        DEFAULT(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/root"), 0,
            Identifier.withDefaultNamespace("textures/block/stone.png")),
        MAGNETIC(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/discover_magnetic_caves"),
            0X060607, Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/block/galena.png")),
        PRIMORDIAL(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/discover_primordial_caves"),
            0XF2D860, Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/block/limestone.png")),
        TOXIC(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/discover_toxic_caves"), 0X7EFF00,
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/block/radrock.png")),
        ABYSSAL(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/discover_abyssal_chasm"), 0X011437,
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/block/abyssmarine.png")),
        FORLORN(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/discover_forlorn_hollows"),
            0X15110E, Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/block/guanostone.png")),
        CANDY(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "alexscaves/discover_candy_cavity"), 0XF795CA,
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/block/block_of_chocolate.png"));

        Identifier root;
        private final int backgroundColor;
        private final Identifier baseStone;
        private final Identifier midground;
        private final Identifier background;

        Type(Identifier root, int backgroundColor, Identifier baseStone) {
            this.root = root;
        ...
        private Identifier generateTexture(String type) {
            return this == DEFAULT ? null : Identifier.fromNamespaceAndPath(
                AlexsCaves.MODID,
        ...
        public static Type forAdvancementHolder(AdvancementHolder holder) {
            return holder == null ? DEFAULT : getDirectType(holder);
        }
