...
/**
 * The "charge cone" indicator that fills while riding a Candicorn / dinosaur mount (and unleashes on the
 * ability key). Ported from AC's NeoForge RenderGuiLayerEvent handler (dropped in this port). Registered
 * via Fabric's 26.1 HudElementRegistry, above the hotbar like the darkness/irradiated meters.
 */
public class RidingMeterHudElement implements HudElement {
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
        ...
        // Frame (unfilled outline) at v = vOffset + dinoHeight, then the fill from the top down proportional
        // to how much charge is MISSING (so the cone empties from the top as it charges).
