...
public class HazmatArmorModel extends HumanoidModel {
    ...
    // 26.1: animate the breathing tanks + mask-follows-head here (driven by the deferred
    // setupAnim(renderState)), replacing the dead NeoForge withAnimations(LivingEntity) hook.
    @Override
    public void setupAnim(net.minecraft.client.renderer.entity.state.HumanoidRenderState state) {
        super.setupAnim(state);
        float f = (Minecraft.getInstance().level == null ? 0F : (float) Minecraft.getInstance().level.getGameTime())
