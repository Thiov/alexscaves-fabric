...
public class LicowitchRenderer extends MobRenderer121X<LicowitchEntity, LicowitchModel> {
    public static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/licowitch.png");
    ...
    public LicowitchRenderer(EntityRendererProvider.Context renderManagerIn) {
        ...
        this.addLayer(new ItemLayer(com.github.alexmodguy.alexscaves.client.render.entity.compat.EntityRendererContextCompat.getItemInHandRenderer(renderManagerIn)));
    ...
    public Identifier getTextureLocation(LicowitchEntity entity) {
        return TEXTURE;
    }

    private class ItemLayer extends ItemInHandLayer121X<LicowitchEntity, LicowitchModel> {
        ...
        protected void renderArmWithItem(LivingEntity livingEntity, ItemStack itemStack, ItemDisplayContext displayContext, HumanoidArm humanoidArm, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight) {
            if (!itemStack.isEmpty() && livingEntity instanceof LicowitchEntity licowitch) {
                ...
                com.github.alexmodguy.alexscaves.client.render.entity.compat.ItemInHandCompat.renderItem(witchItemInHandRenderer, livingEntity, itemStack, displayContext, poseStack, multiBufferSource, packedLight);
                poseStack.popPose();
            }
        }
    }
    ...
    class TeleportingDoubleLayer extends RenderLayer121X<LicowitchEntity, LicowitchModel> {
        ...
        public void render(PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn, LicowitchEntity witch, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            ...
            if (progress > 0.0F) {
                ...
                if (to != null) {
                    ...
                    if (vec3.length() > 0.5F) {
                        ACPostEffectRegistry.renderEffectForNextTick(ClientProxy.PURPLE_WITCH_SHADER);
                        VertexConsumer textureBuffer2 = bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.entityTranslucent(LicowitchRenderer.this.getTextureLocation(witch)));
