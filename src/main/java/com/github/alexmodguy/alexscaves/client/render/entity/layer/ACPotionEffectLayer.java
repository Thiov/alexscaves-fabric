...
public class ACPotionEffectLayer<S extends LivingEntityRenderState, M extends EntityModel<? super S>> extends RenderLayer<S, M> {
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector collector, int lightCoords, S state, float yRot, float xRot) {
        ...
        if (effect.alexscaves$isIrradiated() && glowConfig) {
            ...
            // ACPostEffectRegistry radius-32 blurs that ISOLATED green and composites the soft aura over the
            // scene. The blur of the isolated green IS the radiating aura.
            ...
            // Lower alpha so the radius-32-blurred green composites as a translucent AURA that lets the base
            // player/skin (rendered normally underneath) show through, rather than an opaque green blob.
