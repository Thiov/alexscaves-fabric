...
public class MagneticCavesAmbientParticle extends net.minecraft.client.particle.TextureSheetParticle {
    ...
    public static Vec3 getStartPosition(ClientLevel level, boolean goUp, double x, double y, double z) {
        ...
        if (goUp) {
            while (pos.getY() < level.getMaxY() && level.getBlockState(pos).isAir()) {
                pos = pos.above();
            }
        } else {
