...
public class DarknessArmorModel  extends HumanoidModel {
    ...
    // 26.1: the deferred armor pass calls setupAnim(renderState) on the cached per-slot model at flush
    // time, so animate the custom parts here (the old withAnimations(LivingEntity) was driven by the
    // NeoForge IClientItemExtensions hook Fabric never calls — see ACArmorRenderProperties). This is what
    // brings the cloak's cape (and hood horns) to life instead of leaving them at the stiff default pose.
    @Override
    public void setupAnim(HumanoidRenderState state) {
        super.setupAnim(state);
        float partialTick = Minecraft.getInstance().getDeltaTracker().getGameTimeDeltaPartialTick(false);
        float limbSwingAmount = Math.min(state.walkAnimationSpeed, 1.0F);
        float ageInTicks = (Minecraft.getInstance().level == null ? 0F : (float) Minecraft.getInstance().level.getGameTime()) + partialTick;
    ...
    public DarknessArmorModel withAnimations(LivingEntity entity) {
        float partialTick = Minecraft.getInstance().getDeltaTracker().getRealtimeDeltaTicks();
