...
public class ACModelLayers {
    ...
    public static void registerFabric() {
        ModelLayerRegistry.registerModelLayer(PRIMORDIAL_ARMOR,
                () -> PrimordialArmorModel.createArmorLayer(new CubeDeformation(0.5F)));
        ModelLayerRegistry.registerModelLayer(HAZMAT_ARMOR,
                () -> HazmatArmorModel.createArmorLayer(new CubeDeformation(0.5F)));
        ModelLayerRegistry.registerModelLayer(DIVING_ARMOR,
                () -> DivingArmorModel.createArmorLayer(new CubeDeformation(0.5F)));
        ModelLayerRegistry.registerModelLayer(DARKNESS_ARMOR,
                () -> DarknessArmorModel.createArmorLayer(new CubeDeformation(0.5F)));
        ModelLayerRegistry.registerModelLayer(RAINBOUNCE_ARMOR,
                () -> RainbounceArmorModel.createArmorLayer(new CubeDeformation(0.75F)));
        ModelLayerRegistry.registerModelLayer(GINGERBREAD_ARMOR,
    ...
    private static ModelLayerLocation createLocation(String model, String layer) {
        return new ModelLayerLocation(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, model), layer);
    }
