...
public class HologramProjectorBlockRenderer<T extends HologramProjectorBlockEntity> implements com.github.alexmodguy.alexscaves.client.render.blockentity.compat.BlockEntityRenderer121X<T> {
    ...
    private static int sortBlockPos(Camera camera, BlockPos blockPos1, BlockPos blockPos2) {
        double d1 = camera.position().distanceTo(Vec3.atCenterOf(blockPos1));
        double d2 = camera.position().distanceTo(Vec3.atCenterOf(blockPos2));
    ...
    private static String getPlayerModelName(PlayerInfo playerInfo, UUID uuid) {
        return playerInfo == null ? DefaultPlayerSkin.get(uuid).model().getSerializedName() : playerInfo.getSkin().model().getSerializedName();
    }

    private static PlayerModelType getPlayerSkinModel(PlayerInfo playerInfo, UUID uuid) {
        return playerInfo == null ? DefaultPlayerSkin.get(uuid).model() : playerInfo.getSkin().model();
    }


    private static Identifier getPlayerSkinTextureLocation(PlayerInfo playerInfo, UUID uuid) {
        return playerInfo == null ? DefaultPlayerSkin.get(uuid).body().texturePath() : playerInfo.getSkin().body().texturePath();
    }
    ...
    private static void renderPlayerHologram(UUID lastPlayerUUID, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int i) {
        ...
        PlayerModel model = skinModel == PlayerModelType.SLIM ? slimPlayerModel : playerModel;

        Identifier skinTexture = getPlayerSkinTextureLocation(playerInfo, lastPlayerUUID);
        ...
        poseStack.popPose();
        com.github.alexmodguy.alexscaves.client.render.compat.RenderTargetCompat.bindWrite(Minecraft.getInstance().getMainRenderTarget());
    }
