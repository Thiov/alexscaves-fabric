...
/**
 * 26.1 port notes: the old sprite Material moved to net.minecraft.client.resources.model.sprite.Material,
 * ModelState moved to net.minecraft.client.renderer.block.dispatch.ModelState, and UnbakedModel is now an
 * interface of default methods only. The legacy bake path was already disabled (it is never constructed
 * from live code - TabulaModelHandler's loadModel is commented out), so bake keeps returning null.
 *
 * @author pau101
 * @since 1.0.0
 */
...
/*
    Legacy Tabula block/item model baking, disabled long before the 26.1 port (pre-1.21 model APIs).
